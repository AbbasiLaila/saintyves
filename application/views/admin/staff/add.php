...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      <form role="form"  action="<?php echo base_url('admin/staff/add') ?>" method="post" enctype="multipart/form-data">
                ...
                <label for="department_id">Staff Department</label>
                 <select class="form-control" name="department_id">
                  <?php foreach ($departments as $department) {?>
                    <option  <?php  if(@$_POST['department_id'] == $department['id']){ $checked="selected"; }else{$checked="";} ?> <?=$checked;?>  value="<?=$department['id']?>" ><?=$department['en_name']?></option>
                  <?php } ?>
                  </select>
              </div>
               <p class="help-block red"><?=@$errors['department_id']?></p> 
              <div class="form-group">
                <label for="image">Staff Member Image</label>
      ...
      <div class="loader"><img src="<?=base_url('assets/adminPanel/images/loader.gif')?>" alt=""></div>       
        ...
        <div class="text-center">
          ...
          <a href="<?=base_url('staff')?>" class="btn btn-info">Back</a>
        </div>
      </form>
    </div>
  </div>

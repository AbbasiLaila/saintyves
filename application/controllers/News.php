...
class news extends CI_Controller {
		...
		if($_POST){	
		 		 ...
		 		 if($this->upload_image('image'))
			{
			   $data['image']=$this->upload_image('image'); 
				...
				$this->session->set_flashdata('newsAdded', '');
			...
			if($this->data['upload_response'] == ""){
    			...
    			$return=$this->main_model->insertData("news",$data); 
    			$this->session->set_flashdata('newsAdded', '<div class="alert alert-success" role="alert">   <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                News post has been added successfully.</div>'); 
				redirect('admin/news');
    		}
	...
	{		
	if($this->main_model->check($id, "news")==false){
		...
		if($_POST){	 
		 		 ...
		 		 if($this->upload_image('image'))
			{
			   $data['image']=$this->upload_image('image'); 
			    ...
			    $this->data['upload_response'] =  $this->upload->display_errors();
			$this->session->set_flashdata('newsEdited', ''); 
			...
			$return=$this->main_model->updateData($id,"news",$data); //submit to database
			$this->session->set_flashdata('newsEdited', '<div class="alert alert-success" role="alert">   <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
                News Post has been edited successfully</div>'); 
		}
	}
		...
		$this->data['post']=$this->main_model->getItem('news',$id); 
...
public function delete($id)
	{
		if($this->main_model->check($id, "news")==false){
			...
			$this->main_model->deleteData('news',$id);
		}
	}
